...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="mt-1 text-gray-600">Isi form pengajuan untuk diproses oleh admin TU.</p>
        </div>
    </div>

    <x-form-draft formId="create-izin-karyawan" formName="Izin Karyawan">
    <div class="rounded-xl bg-white p-6 shadow-sm">
        <form id="create-izin-karyawan" action="{{ route('kepegawaian-tu.izin-karyawan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6"
              x-data="{ jenis: @js(old('jenis')) }">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="nama_karyawan" id="nama_karyawan" value="{{ old('nama_karyawan', auth()->user()->name) }}" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('nama_karyawan') border-red-500 @enderror">
                    @error('nama_karyawan')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai') }}" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('tanggal_mulai') border-red-500 @enderror">
                    @error('tanggal_mulai')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('tanggal_selesai') border-red-500 @enderror">
            ...
            <div x-show="jenis === 'dinas_luar'" x-cloak class="grid grid-cols-1 gap-6 rounded-lg border border-blue-100 bg-blue-50 p-4 sm:grid-cols-3">
                <div>
                    ...
                    <input type="text" name="dinas_luar_hari" id="dinas_luar_hari" value="{{ old('dinas_luar_hari') }}" placeholder="Contoh: Senin" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('dinas_luar_hari') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="dinas_luar_waktu" id="dinas_luar_waktu" value="{{ old('dinas_luar_waktu') }}" placeholder="Contoh: 08.00 - 16.00 WIB" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('dinas_luar_waktu') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="dinas_luar_tempat" id="dinas_luar_tempat" value="{{ old('dinas_luar_tempat') }}" placeholder="Contoh: Dinas Pendidikan Kota Bogor" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('dinas_luar_tempat') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea name="alasan" id="alasan" rows="4" class="w-full rounded-lg border border-gray-300 px-4 py-2 @error('alasan') border-red-500 @enderror">{{ old('alasan') }}</textarea>
                @error('alasan')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 border-t pt-4">
                <a href="{{ route('kepegawaian-tu.izin-karyawan.index') }}" class="rounded-lg bg-gray-100 px-4 py-2 text-gray-700 hover:bg-gray-200 transition">Batal</a>
                <button type="submit" class="rounded-lg bg-slate-700 px-4 py-2 text-white hover:bg-slate-800 transition">Kirim Pengajuan</button>
            </div>
        </form>
    </div>
    </x-form-draft>
