...
<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Audit Log TU</h1>
        <p class="mt-1 text-gray-600">Jejak aktivitas untuk modul Kepegawaian TU (create/update/delete/approval/export).</p>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-4">
        <form action="{{ route('kepegawaian-tu.audit-log.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <select name="user_id" class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm sm:w-auto">
            ...
            @if(request()->filled('module') || request()->filled('action') || request()->filled('user_id'))
                <a href="{{ route('kepegawaian-tu.audit-log.index') }}" class="rounded-lg bg-gray-100 px-4 py-2 text-gray-700 hover:bg-gray-200 transition">Reset</a>
            @endif
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-gray-500">Belum ada log aktivitas TU.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
