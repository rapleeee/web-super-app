...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('sarana-umum.data-guru.create') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
        <form action="{{ route('sarana-umum.data-guru.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            @if(request()->hasAny(['search', 'status']))
                <a href="{{ route('sarana-umum.data-guru.index') }}" class="px-4 py-2 text-gray-600 hover:text-gray-900">
                    Reset
                </a>
            @endif
        </form>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('sarana-umum.data-guru.edit', $guru) }}" class="text-yellow-600 hover:text-yellow-800" title="Edit">
                                        <x-heroicon-o-pencil-square class="w-5 h-5"/>
                                    </a>
                                    <form id="delete-guru-{{ $guru->id }}" action="{{ route('sarana-umum.data-guru.destroy', $guru) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
