    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <!-- Download Backup -->
                    <a href="{{ route('sarana-umum.backup.download') }}" 
                    ...
                    <!-- Save to Server -->
                    <form action="{{ route('sarana-umum.backup.store') }}" method="POST">
                        @csrf
                        <button type="submit" 
                        ...
                        @foreach($existingBackups as $backup)
                            <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('sarana-umum.backup.download-file', $backup['name']) }}" 
                                       class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"
                                       title="Download">
                                        <x-heroicon-o-arrow-down-tray class="w-5 h-5"/>
                                    </a>
                                    <form id="delete-backup-{{ md5($backup['name']) }}" action="{{ route('sarana-umum.backup.destroy', $backup['name']) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                                onclick="confirmDelete('delete-backup-{{ md5($backup['name']) }}', 'Hapus Backup?', 'File backup {{ $backup['name'] }} akan dihapus permanen!')"
