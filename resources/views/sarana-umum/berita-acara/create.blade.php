...
<div class="max-w-full mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Catat penggunaan sarana umum</p>
        </div>
    </div>

    <x-form-draft formId="create-berita-acara-sarana" formName="Berita Acara Sarana Umum">
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form id="create-berita-acara-sarana" action="{{ route('sarana-umum.berita-acara.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div>
                    ...
                    <select name="ruangan_id" id="ruangan_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('ruangan_id') border-red-500 @enderror">
                        ...
                        @foreach ($ruangans as $ruangan)
                            <option value="{{ $ruangan->id }}" {{ old('ruangan_id') == $ruangan->id ? 'selected' : '' }}>{{ $ruangan->nama }} ({{ $ruangan->kode }})</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('tanggal') border-red-500 @enderror">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="time" name="waktu_mulai" id="waktu_mulai" value="{{ old('waktu_mulai') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('waktu_mulai') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="time" name="waktu_selesai" id="waktu_selesai" value="{{ old('waktu_selesai') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('waktu_selesai') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="nama_guru" id="nama_guru" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('nama_guru') border-red-500 @enderror">
                        ...
                        @foreach ($gurus as $guru)
                            <option value="{{ $guru->nama }}" {{ old('nama_guru') === $guru->nama ? 'selected' : '' }}>{{ $guru->nama }}{{ $guru->nip ? ' (' . $guru->nip . ')' : '' }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="mata_pelajaran" id="mata_pelajaran" value="{{ old('mata_pelajaran') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('mata_pelajaran') border-red-500 @enderror">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" name="kelas" id="kelas" value="{{ old('kelas') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('kelas') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_peserta" id="jumlah_peserta" value="{{ old('jumlah_peserta') }}" min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('jumlah_peserta') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea name="kegiatan" id="kegiatan" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('kegiatan') border-red-500 @enderror">{{ old('kegiatan') }}</textarea>
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg @error('catatan') border-red-500 @enderror">{{ old('catatan') }}</textarea>
                @error('catatan')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-center justify-end gap-3 pt-4 border-t">
                <a href="{{ route('sarana-umum.berita-acara.index') }}" class="px-4 py-2 text-gray-700 hover:text-gray-900">Batal</a>
                <button type="submit" class="px-6 py-2 bg-[#272125] text-white rounded-lg hover:bg-[#3a3136] transition">Simpan</button>
            </div>
        </form>
    </div>
    </x-form-draft>
