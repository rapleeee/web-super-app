...
<div class="max-w-full mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Perbarui data ruangan</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('sarana-umum.data-ruangan.update', $ruangan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode" id="kode" value="{{ old('kode', $ruangan->kode) }}"
                ...
                {{-- Nama --}}
                <div>
                    <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama Ruangan <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $ruangan->kapasitas) }}" min="1"
                ...
                <div>
                    ...
                    <select name="jurusan" id="jurusan"
                        ...
                        <option value="RPL" {{ old('jurusan', $ruangan->jurusan) === 'RPL' ? 'selected' : '' }}>RPL</option>
                        <option value="TKJ" {{ old('jurusan', $ruangan->jurusan) === 'TKJ' ? 'selected' : '' }}>TKJ</option>
                        <option value="DKV" {{ old('jurusan', $ruangan->jurusan) === 'DKV' ? 'selected' : '' }}>DKV</option>
                    </select>
                    @error('jurusan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="renovasi" {{ old('status', $ruangan->status) === 'renovasi' ? 'selected' : '' }}>Renovasi</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select name="penanggung_jawab_id" id="penanggung_jawab_id"
                    ...
                    @foreach ($petugasLaboran as $petugas)
                        <option value="{{ $petugas->id }}" {{ old('penanggung_jawab_id', $ruangan->penanggung_jawab_id) == $petugas->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                          ...
                          placeholder="Deskripsi ruangan...">{{ old('deskripsi', $ruangan->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @php
                        ...
                        $oldFasilitas = old('fasilitas', $ruangan->fasilitas ?? []);
                    @endphp
            ...
            {{-- Foto --}}
            <div>
                <label for="foto" class="block text-sm font-medium text-gray-700 mb-2">Foto Ruangan</label>
                @if ($ruangan->foto)
                    <div class="mb-3">
                        <img src="{{ Storage::url($ruangan->foto) }}" alt="{{ $ruangan->nama }}" class="w-32 h-20 rounded-lg object-cover">
                ...
                <p class="mt-1 text-xs text-gray-500">Format: JPG, JPEG, PNG. Maks: 2MB. Kosongkan jika tidak ingin mengubah foto.</p>
                @error('foto')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t">
                <a href="{{ route('sarana-umum.data-ruangan.index') }}" class="px-4 py-2 text-gray-700 hover:text-gray-900">
                    Batal
                </a>
