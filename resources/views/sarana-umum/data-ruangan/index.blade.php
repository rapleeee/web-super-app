...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Data Ruangan</h1>
            <p class="text-gray-600 mt-1">Kelola data ruangan</p>
        </div>
        <a href="{{ route('sarana-umum.data-ruangan.create') }}"
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('sarana-umum.data-ruangan.show', $lab) }}" class="text-blue-600 hover:text-blue-800" title="Lihat">
                                        <x-heroicon-o-eye class="w-5 h-5"/>
                                    </a>
                                    <a href="{{ route('sarana-umum.data-ruangan.edit', $lab) }}" class="text-yellow-600 hover:text-yellow-800" title="Edit">
                                        <x-heroicon-o-pencil-square class="w-5 h-5"/>
                                    </a>
                                    <form id="delete-lab-{{ $lab->id }}" action="{{ route('sarana-umum.data-ruangan.destroy', $lab) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete('delete-lab-{{ $lab->id }}', 'Hapus Ruangan?', 'Data {{ $lab->nama }} akan dihapus permanen!')" class="text-red-600 hover:text-red-800" title="Hapus">
