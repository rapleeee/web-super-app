...
<div class="max-w-full mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Unit Komputer</h1>
            <p class="text-gray-600 mt-1">{{ $unitKomputer->nama }}</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('laboran.unit-komputer.update', $unitKomputer) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode_unit" id="kode_unit" value="{{ old('kode_unit', $unitKomputer->kode_unit) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $unitKomputer->nama) }}"
                ...
                <div>
                    ...
                    <select name="laboratorium_id" id="laboratorium_id"
                        ...
                        @foreach ($laboratoriums as $lab)
                            <option value="{{ $lab->id }}" {{ old('laboratorium_id', $unitKomputer->laboratorium_id) == $lab->id ? 'selected' : '' }}>
                                {{ $lab->nama }} ({{ $lab->kode }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="nomor_meja" id="nomor_meja" value="{{ old('nomor_meja', $unitKomputer->nomor_meja) }}" min="1" max="100"
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="kondisi" id="kondisi"
                            ...
                            <option value="baik" {{ old('kondisi', $unitKomputer->kondisi) === 'baik' ? 'selected' : '' }}>Baik</option>
                            <option value="rusak_ringan" {{ old('kondisi', $unitKomputer->kondisi) === 'rusak_ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                            <option value="rusak_berat" {{ old('kondisi', $unitKomputer->kondisi) === 'rusak_berat' ? 'selected' : '' }}>Rusak Berat</option>
                            <option value="mati_total" {{ old('kondisi', $unitKomputer->kondisi) === 'mati_total' ? 'selected' : '' }}>Mati Total</option>
                        </select>
                        @error('kondisi')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="aktif" {{ old('status', $unitKomputer->status) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="dalam_perbaikan" {{ old('status', $unitKomputer->status) === 'dalam_perbaikan' ? 'selected' : '' }}>Dalam Perbaikan</option>
                            <option value="tidak_aktif" {{ old('status', $unitKomputer->status) === 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#272125] focus:border-transparent @error('keterangan') border-red-500 @enderror">{{ old('keterangan', $unitKomputer->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
