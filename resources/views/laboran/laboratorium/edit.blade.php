...
<div class="max-w-full mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Perbarui data laboratorium</p>
        </div>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('laboran.laboratorium.update', $laboratorium) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode" id="kode" value="{{ old('kode', $laboratorium->kode) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $laboratorium->nama) }}"
            ...
            <div>
                ...
                <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $laboratorium->lokasi) }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $laboratorium->kapasitas) }}" min="1"
                ...
                <div>
                    ...
                    <select name="jurusan" id="jurusan"
                        ...
                        <option value="RPL" {{ old('jurusan', $laboratorium->jurusan) === 'RPL' ? 'selected' : '' }}>RPL</option>
                        <option value="TKJ" {{ old('jurusan', $laboratorium->jurusan) === 'TKJ' ? 'selected' : '' }}>TKJ</option>
                        <option value="DKV" {{ old('jurusan', $laboratorium->jurusan) === 'DKV' ? 'selected' : '' }}>DKV</option>
                    </select>
                    @error('jurusan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="nonaktif" {{ old('status', $laboratorium->status) === 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        <option value="renovasi" {{ old('status', $laboratorium->status) === 'renovasi' ? 'selected' : '' }}>Renovasi</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select name="penanggung_jawab_id" id="penanggung_jawab_id"
                    ...
                    @foreach ($petugasLaboran as $petugas)
                        <option value="{{ $petugas->id }}" {{ old('penanggung_jawab_id', $laboratorium->penanggung_jawab_id) == $petugas->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                          ...
                          placeholder="Deskripsi laboratorium...">{{ old('deskripsi', $laboratorium->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    @php
                        ...
                        $oldFasilitas = old('fasilitas', $laboratorium->fasilitas ?? []);
                    @endphp
            ...
            <div>
                ...
                @if ($laboratorium->foto)
                    <div class="mb-3">
                        <img src="{{ Storage::url($laboratorium->foto) }}" alt="{{ $laboratorium->nama }}" class="w-32 h-20 rounded-lg object-cover">
